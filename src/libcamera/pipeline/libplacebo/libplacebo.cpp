...
LOG_DEFINE_CATEGORY(LibplaceboPipeline)
 ...
 * --------
 *
 * The LibplaceboPipelineHandler relies on generic kernel APIs to control a camera
 ...
 * any of the cameras and stores them in LibplaceboPipelineHandler::entities_,
 * accessible by the LibplaceboCameraData class through the
 * LibplaceboPipelineHandler::subdev() and LibplaceboPipelineHandler::video() functions.
 ...
 * LibplaceboCameraData::entities_ as a list of LibplaceboCameraData::Entity structures,
 ...
 * the LibplaceboCameraData::Configuration structure, stored in the
 * LibplaceboCameraData::configs_ vector.
 *
 ...
 * outputSizes of the LibplaceboCameraData::Configuration structure.
 *
 ...
 * distinct.
 *
 * A resource reservation mechanism is implemented by the LibplaceboPipelineHandler
...
class LibplaceboPipelineHandler;

struct LibplaceboPipelineInfo {
	const char *driver;
	/*
...
static const LibplaceboPipelineInfo supportedDevices[] = {
	{ "dcmipp", {}, false },
...
class LibplaceboCameraData : public Camera::Private
{
public:
	LibplaceboCameraData(LibplaceboPipelineHandler *pipe,
			 unsigned int numStreams,
			 MediaEntity *sensor);

	bool isValid() const { return sensor_ != nullptr; }
	LibplaceboPipelineHandler *pipe();
...
class LibplaceboCameraConfiguration : public CameraConfiguration
{
public:
	LibplaceboCameraConfiguration(Camera *camera, LibplaceboCameraData *data);

	Status validate() override;

	const LibplaceboCameraData::Configuration *pipeConfig() const
	{
		return pipeConfig_;
	}
...
private:
	/*
	 * The LibplaceboCameraData instance is guaranteed to be valid as long as
	 * the corresponding Camera instance is valid. In order to borrow a
	 * reference to the camera data, store a new reference to the camera.
	 */
	std::shared_ptr<Camera> camera_;
	LibplaceboCameraData *data_;

	const LibplaceboCameraData::Configuration *pipeConfig_;
	bool needConversion_;
	Transform combinedTransform_;
};

class LibplaceboPipelineHandler : public PipelineHandler
{
public:
	LibplaceboPipelineHandler(CameraManager *manager);
...
private:
	...
	struct EntityData {
		...
		std::map<const MediaPad *, LibplaceboCameraData *> owners;
	};

	LibplaceboCameraData *cameraData(Camera *camera)
	{
		return static_cast<LibplaceboCameraData *>(camera->_d());
	}

	std::vector<MediaEntity *> locateSensors(MediaDevice *media);
	static int resetRoutingTable(V4L2Subdevice *subdev);

	const MediaPad *acquirePipeline(LibplaceboCameraData *data);
	void releasePipeline(LibplaceboCameraData *data);
...
LibplaceboCameraData::LibplaceboCameraData(LibplaceboPipelineHandler *pipe,
				   unsigned int numStreams,
				   MediaEntity *sensor)
...
LibplaceboPipelineHandler *LibplaceboCameraData::pipe()
{
	return static_cast<LibplaceboPipelineHandler *>(Camera::Private::pipe());
}

int LibplaceboCameraData::init()
{
	LibplaceboPipelineHandler *pipe = LibplaceboCameraData::pipe();
	int ret;
	...
	if (converter) {
		...
		if (!converter_) {
			LOG(LibplaceboPipeline, Warning)
				<< "Failed to create converter, disabling format conversion";
			converter_.reset();
		} else {
			converter_->inputBufferReady.connect(this, &LibplaceboCameraData::conversionInputDone);
			converter_->outputBufferReady.connect(this, &LibplaceboCameraData::conversionOutputDone);
		}
	}
	...
	if (!converter_ && pipe->swIspEnabled()) {
		...
		if (!swIsp_->isValid()) {
			 ...
			 * LibplaceboCameraData class doesn't inherit from the Object class, it
			...
			swIsp_->outputBufferReady.connect(this, &LibplaceboCameraData::conversionOutputDone);
			swIsp_->ispStatsReady.connect(this, &LibplaceboCameraData::ispStatsReady);
			swIsp_->setSensorControls.connect(this, &LibplaceboCameraData::setSensorControls);
		}
	}
	...
	if (configs_.empty()) {
		LOG(LibplaceboPipeline, Error) << "No valid configuration found";
		return -EINVAL;
	}
 ...
 */
void LibplaceboCameraData::tryPipeline(unsigned int code, const Size &size)
{
	/*
...
int LibplaceboCameraData::setupLinks()
{
	int ret;
	...
	for (LibplaceboCameraData::Entity &e : entities_) {
		if (!sinkLink) {
...
int LibplaceboCameraData::setupFormats(V4L2SubdeviceFormat *format,
				   V4L2Subdevice::Whence whence,
				   Transform transform)
{
	LibplaceboPipelineHandler *pipe = LibplaceboCameraData::pipe();
...
void LibplaceboCameraData::imageBufferReady(FrameBuffer *buffer)
{
	LibplaceboPipelineHandler *pipe = LibplaceboCameraData::pipe();
...
void LibplaceboCameraData::clearIncompleteRequests()
{
...
void LibplaceboCameraData::conversionInputDone(FrameBuffer *buffer)
{
	/* Queue the input buffer back for capture. */
	video_->queueBuffer(buffer);
}

void LibplaceboCameraData::conversionOutputDone(FrameBuffer *buffer)
{
	LibplaceboPipelineHandler *pipe = LibplaceboCameraData::pipe();
...
void LibplaceboCameraData::ispStatsReady(uint32_t frame, uint32_t bufferId)
{
	swIsp_->processStats(frame, bufferId,
			     delayedCtrls_->get(frame));
}

void LibplaceboCameraData::setSensorControls(const ControlList &sensorControls)
{
...
std::vector<const MediaPad *> LibplaceboCameraData::routedSourcePads(MediaPad *sink)
{
...
LibplaceboCameraConfiguration::LibplaceboCameraConfiguration(Camera *camera,
						     LibplaceboCameraData *data)
...
CameraConfiguration::Status LibplaceboCameraConfiguration::validate()
{
	 ...
	 * format.
	 */
	const std::vector<const LibplaceboCameraData::Configuration *> *configs =
	 ...
	 * without upscaling.
	 */
	const LibplaceboCameraData::Configuration *maxPipeConfig = nullptr;
	pipeConfig_ = nullptr;

	for (const LibplaceboCameraData::Configuration *pipeConfig : *configs) {
	...
	for (unsigned int i = 0; i < config_.size(); ++i) {
		...
		if (cfg.pixelFormat != pixelFormat) {
			LOG(LibplaceboPipeline, Debug) << "Adjusting pixel format";
...
LibplaceboPipelineHandler::LibplaceboPipelineHandler(CameraManager *manager)
	: PipelineHandler(manager), converter_(nullptr)
{
}

std::unique_ptr<CameraConfiguration>
LibplaceboPipelineHandler::generateConfiguration(Camera *camera, Span<const StreamRole> roles)
{
	LibplaceboCameraData *data = cameraData(camera);
	std::unique_ptr<CameraConfiguration> config =
		std::make_unique<LibplaceboCameraConfiguration>(camera, data);
	...
	for (const LibplaceboCameraData::Configuration &cfg : data->configs_) {
...
int LibplaceboPipelineHandler::configure(Camera *camera, CameraConfiguration *c)
{
	LibplaceboCameraConfiguration *config =
		static_cast<LibplaceboCameraConfiguration *>(c);
	LibplaceboCameraData *data = cameraData(camera);
	...
	const LibplaceboCameraData::Configuration *pipeConfig = config->pipeConfig();
	V4L2SubdeviceFormat format{};
...
int LibplaceboPipelineHandler::exportFrameBuffers(Camera *camera, Stream *stream,
...
int LibplaceboPipelineHandler::start(Camera *camera, [[maybe_unused]] const ControlList *controls)
{
	LibplaceboCameraData *data = cameraData(camera);
	V4L2VideoDevice *video = data->video_;
	int ret;
	...
	video->bufferReady.connect(data, &LibplaceboCameraData::imageBufferReady);
...
void LibplaceboPipelineHandler::stopDevice(Camera *camera)
{
	LibplaceboCameraData *data = cameraData(camera);
	V4L2VideoDevice *video = data->video_;
	...
	video->bufferReady.disconnect(data, &LibplaceboCameraData::imageBufferReady);
...
int LibplaceboPipelineHandler::queueRequestDevice(Camera *camera, Request *request)
{
	LibplaceboCameraData *data = cameraData(camera);
	int ret;
...
LibplaceboPipelineHandler::locateSensors(MediaDevice *media)
{
...
int LibplaceboPipelineHandler::resetRoutingTable(V4L2Subdevice *subdev)
{
...
bool LibplaceboPipelineHandler::match(DeviceEnumerator *enumerator)
{
	const LibplaceboPipelineInfo *info = nullptr;
	unsigned int numStreams = 1;
	MediaDevice *media;

	for (const LibplaceboPipelineInfo &inf : supportedDevices) {
	...
	if (sensors.empty()) {
		LOG(LibplaceboPipeline, Info) << "No sensor found for " << media->deviceNode();
		return false;
	}

	LOG(LibplaceboPipeline, Debug) << "Sensor found for " << media->deviceNode();

	/*
	 * Create one camera data instance for each sensor and gather all
	 * entities in all pipelines.
	 */
	std::vector<std::unique_ptr<LibplaceboCameraData>> pipelines;
	std::set<MediaEntity *> entities;

	pipelines.reserve(sensors.size());

	for (MediaEntity *sensor : sensors) {
		std::unique_ptr<LibplaceboCameraData> data =
			std::make_unique<LibplaceboCameraData>(this, numStreams, sensor);
		if (!data->isValid()) {
			LOG(LibplaceboPipeline, Error)
		...
		for (LibplaceboCameraData::Entity &entity : data->entities_)
	...
	for (std::unique_ptr<LibplaceboCameraData> &data : pipelines) {
		int ret = data->init();
		if (ret < 0)
			continue;
...
V4L2VideoDevice *LibplaceboPipelineHandler::video(const MediaEntity *entity)
{
...
V4L2Subdevice *LibplaceboPipelineHandler::subdev(const MediaEntity *entity)
{
	auto iter = entities_.find(entity);
	if (iter == entities_.end())
		return nullptr;
 ...
 */
const MediaPad *LibplaceboPipelineHandler::acquirePipeline(LibplaceboCameraData *data)
{
	for (const LibplaceboCameraData::Entity &entity : data->entities_) {
...
void LibplaceboPipelineHandler::releasePipeline(LibplaceboCameraData *data)
{
	for (const LibplaceboCameraData::Entity &entity : data->entities_) {
		EntityData &edata = entities_[entity.entity];
...
REGISTER_PIPELINE_HANDLER(LibplaceboPipelineHandler, "libplacebo")
