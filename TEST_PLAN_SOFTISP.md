...
**Target:** `virtual_softisp` pipeline (No hardware required)

---

## 1. Objective
Verify that the **SoftISP** Image Processing Algorithm (IPA) functions correctly within the **libcamera** framework using the `virtual_softisp` pipeline. This test validates the entire stack (Pipeline → IPA Module → ONNX Models) **without requiring any physical camera hardware**.
...
### 3.2. Configure Meson
We enable the SoftISP feature, explicitly request both pipelines (`softisp` and `virtual_softisp`), and disable strict error checking to bypass known Termux-specific build warnings.

```bash
meson setup build \
  -Dsoftisp=enabled \
  '-Dpipelines=simple,softisp,virtual_softisp' \
  -Dtest=true \
  -Dc_args=-Wno-error \
  -Dcpp_args=-Wno-error
```
...
- `softisp_virtual_module_test`: Passes. Output: `name = softisp-virtual, pipelineName = virtual_softisp`

### 4.3. Step 2: Integration Test (Virtual Pipeline)
Run the test application with the `virtual_softisp` pipeline to simulate a camera session.

```bash
./tools/softisp-test-app --pipeline virtual_softisp --output test_virtual.yuv --frames 10
```

**✅ Expected Output:**
```text
Available cameras:
  /dev/media0 (Pipeline: virtual_softisp)
Using camera: /dev/media0 (Pipeline: virtual_softisp)
...
| :--- | :--- | :--- |
| `No camera found for pipeline` | Pipeline not built or not in path | Ensure `-Dpipelines=...,virtual_softisp` was used. Check `build/src/libcamera/pipeline/virtual_softisp/`. |
...
3.  `softisp-test-app` runs successfully with `--pipeline virtual_softisp` **without a physical camera**.
